describe('scope 대해서 학습합니다.', function () {
  ...
  it('함수 선언식(declaration)과 함수 표현식(expression)의 차이를 확인합니다.', function () {
    ...
    function funcDeclared() {
      return 'this is a function declaration';  //선언식이면 함수타입
    }

    funcExpressed = function () {
      return 'this is a function expression';   //표현식이면 문자열
    };
  ...
  it('클로저(closure)에 대해 확인합니다.', function () {
    ...
    //increaseBy3(10) = increaseBy(3)(10)
  ...
  it('lexical scope와 closure에 대해 다시 확인합니다.', function () {
    ...
    function outerFn() {
      ...
      expect(age).to.equal(26);           //inner에서 age는 26으로 재할당이 됨
      expect(name).to.equal('jimin');     //outer에서 name이 jimin으로 재할당 됨 inner에서는 let으로 재선언 되어서 값이 반환되지 않음
